...
class TestAsyncCallersOffload:
    @pytest.mark.asyncio
    async def test_log_level_put_runs_save_off_the_event_loop(self, monkeypatch):
        """The #4371 revert reason, as a runtime probe on a converted handler:
        the handler's persist (including the now-locking save) must run in a
        ...
        class _Cfg:
            def __init__(self) -> None:
                self.agent = type("A", (), {"log_level": "INFO"})()

            def save(self) -> None:
                seen["save_on_loop"] = _on_running_loop()

        class _FakeConfig:
            @staticmethod
            def load() -> "_Cfg":
                seen["load_on_loop"] = _on_running_loop()
                return _Cfg()

        monkeypatch.setattr(updates, "KiroCrewConfig", _FakeConfig)
        ...
        assert (
            seen.get("save_on_loop") is False
        ), "api_log_level ran the locking save() inline on the event loop"
        assert (
            seen.get("load_on_loop") is False
        ), "the whole load-mutate-save should run in the worker, off the loop"
