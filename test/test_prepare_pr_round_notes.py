...
def _slug(text: str) -> str:
    return "{}-{}".format(
        text.replace("/", "-"), hashlib.sha1(text.encode("utf-8")).hexdigest()[:8]
    )


def _note(home: Path, repo: str = "acme/widgets", branch: str = "feat/x") -> Path:
    return home / "prepare-pr" / _slug(repo) / (_slug(branch) + ".md")
...
def test_distinct_branches_that_sanitise_alike_get_distinct_notes(tmp_path):
    env = dict(os.environ, KIROCREW_HOME=str(tmp_path))
    for branch in ("feat/x", "feat-x"):
        proc = subprocess.run(
            [
                sys.executable,
                SCRIPT,
        ...
        assert proc.returncode == 0, proc.stderr
    a = _note(tmp_path, branch="feat/x")
    b = _note(tmp_path, branch="feat-x")
    assert a != b and a.exists() and b.exists()
    assert "intent for feat/x" in a.read_text(encoding="utf-8")
    assert "intent for feat-x" in b.read_text(encoding="utf-8")


def test_prune_tolerates_a_note_removed_by_a_sibling(tmp_path, monkeypatch):
    """Two loops pruning at once: a path listed by one and unlinked by the other
    must not raise in the first."""
    sys.path.insert(0, str(Path(SCRIPT).parent))
    import importlib

    rn = importlib.import_module("round_notes")
    monkeypatch.setenv("KIROCREW_HOME", str(tmp_path))
    d = tmp_path / "prepare-pr" / "r"
    ...
    def racing_stat(self, *a, **kw):
        if self == stale:
            stale.unlink()  # the sibling got there first
            return real_stat(self, *a, **kw)
        return real_stat(self, *a, **kw)

    monkeypatch.setattr(Path, "stat", racing_stat)
    ns = type("NS", (), {"days": 14})()
    assert rn.cmd_prune(ns) == 0
