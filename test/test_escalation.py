...
def test_two_addressable_sessions_titled_user_are_refused_not_misrouted(tmp_path):
    """Regression: the reserved-target collision guard resolves the literal
    "user" through the full target-authorization path, so an inaccessible
    same-named session no longer blocks or leaks. But a GENUINE ambiguity —
    two DIFFERENT addressable sessions both titled "user" — must still be
    refused, not swallowed and misrouted to the human. `authorize_target`
    raises `ambiguous_target`; the guard re-raises it rather than treating it
    as "no collision"."""
    state = _make_state(tmp_path)
    member = _member_slot(state)
    one = state.get_or_create_slot("chat-2")
    one.title = "user"
    two = state.get_or_create_slot("chat-3")
    two.title = "user"
    with pytest.raises(sc.SessionControlError) as exc:
        _escalate(state, member)
    assert exc.value.code == "ambiguous_target"
    assert exc.value.status == 409
    # No misroute: neither the human-owning member thread nor either titled
    # session received the escalation row.
