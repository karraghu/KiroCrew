...
1. **The subagent timeout is configurable but unbounded.** `agent.subagent_timeout_secs`
   (default 3 h, `_TIMEOUT_SECS` as the fallback) feeds the manager's `default_timeout`, so
   an operator can lengthen it for large code generation or complex multi-tool workflows.
   Unlike the turn budget and the concurrency cap it carries no load-time clamp and is
   absent from the config PUT whitelist, so it is settable only from the CLI or
   `config.json` and an inflated on-disk value is honored as written.
