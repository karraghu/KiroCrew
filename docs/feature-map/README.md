...
| Crew Members | One durable pinned DM thread per crew member; the detail drawer lists the worker sessions the member is driving (live `slots` frames filtered on `created_by`); the roster carries a per-crew star (persisted on the crew record) and persistent filters — starred-only and origin (mine / built-in / from packages) — so the package-installed crews the agent sync writes can be collapsed | `/members` — rail row when the crew preview is on | `pages/members/MembersPage.tsx` | `handlers/members.py`, `handlers/agents.py` (`starred`), `slot_projection.py` (`created_by`) | `GET /api/members`, `POST /api/members/{slug}/thread`, `GET /api/members/{slug}/activity`, `GET,PUT /api/members/{slug}/rules`, `PUT /api/agents/{name}` (`starred`), `GET /api/ws` (`slots`) |
