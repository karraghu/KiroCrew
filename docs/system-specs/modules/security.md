...
- **Structure passes are OFF under an OS-confined shell** (`os_confined`, `_structure_pass_results`, `sandbox.agent_shell_os_confined`): passes 3 to 5 — the native-shell entry-then-relative-read scan and the two traversal analyses — reconstruct which files a command will OPEN from the shape of its command line. That is the question a kernel path deny answers exactly and for every spelling, so when an OS sandbox already confines the agent's shell child the three passes add wall-clock and false refusals while catching nothing the kernel does not. The false refusals are structural, not incidental: `_alt_root_reaching_fence` resolves a relative root such as `.` through `path_contains_sensitive` against the GATEWAY process's own directory before it ever consults a `cd` base on the same line, and the desktop app's gateway runs with `/` as that directory — which holds every fenced store — so every `grep -r … .` / `rg … .` was refused regardless of the `cd` that preceded it. The gate therefore takes `os_confined` and, when true, iterates an empty pass set in place of `_structure_pass_results` (the three passes grouped behind one lazy generator, first denial wins, so a later pass never runs after an earlier refusal — exactly as they ran inline). Everything else stays on: passes 0 to 2 judge paths the command NAMES (the keystone leaves and credential homes by literal, collapsed or normalized spelling — pass 2's own `cd` walk included — which is what keeps the governance ceiling covered under a Seatbelt tier that exposes third-party credential homes by design), and the IMDS and env-credential rules describe behaviour no path deny sees. The default is the full analysis, so a caller that does not know its posture — cron command vetting, computer-use input vetting, the source-body scanner — keeps every pass; the two agent-shell gates (`hooks.on_tool_call` and `_resolve_permission`'s off-loop title and tool_input tiers) read the posture once per call, on the worker hop for the latter because the first read may probe the backend. `sandbox.agent_shell_os_confined()` mirrors `wrap_argv`'s own decision from the same inputs: a real `agent.sandbox` tier with a detected backend confines (Kiro Crew's namespace or Seatbelt profile, or kiro-cli's internal sandbox by the macOS mutual-exclusion delegation); `"off"` confines only where `wrap_argv` delegates — a harness in `ACP_BACKENDS_INTERNAL_SANDBOX` (positive membership, harness-parity H7) on macOS or Windows with `kiro_internal_sandbox_enabled()` true — and never on Linux, which does not delegate. Every failure in that read answers `False`, keeping the full analysis on: an unreadable setting can only ADD checks. **Stated residual**: under the `standard` tier `~/.aws`, `~/.ssh` and `~/.kube` are exposed by design, so an INDIRECT read of those (a traversal rooted at `~`) is then the operator's tier choice rather than this gate's catch; the literal and normalized spellings of those paths are still refused, and the crew trust root is OS-denied in every tier. Pinned in both directions by `test/test_security_os_confined_gate.py`: each structure-only command is asserted refused unconfined AND allowed confined, each named-path or behaviour command is asserted refused identically in both postures, and the posture predicate's decision table is pinned with every input monkeypatched.
