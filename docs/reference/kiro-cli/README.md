...
| [chat/voice.md](chat/voice.md) | **Not a mirror**, and the one page here with no upstream source: Kiro Crew's own voice setup, speech-to-text in (local by default) and text-to-speech out (local Piper, or Amazon Polly). Its behavioral contracts live in [stt-streaming](../../system-specs/features/stt-streaming.md) and [voice-streaming](../../system-specs/features/voice-streaming.md); its settings reference is [configuration](../../../src/kiro_crew/docs/configuration.md). |
