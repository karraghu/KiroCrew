...
export default function McpInfoButton({ agent }: { agent?: string }) {
  ...
  // (`enabled` flips with `open`; stale-by-default), so the answer is fresh
  // without a hand-rolled effect. Same keys as ChatPage's MCP dropdown, so the
  // two surfaces share one cache. A refused read is `isError` — previously a
  // failed fetch left the popover on "None loaded", a claim about the session
