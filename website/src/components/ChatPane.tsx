...
import { readPaneDraft, writePaneDraft, mergePaneDraft } from '../utils/chatPaneDrafts'
...
export default function ChatPane({
  ...
  // Upload failures, keyed by the slot whose files failed. A banner belongs to
  // the conversation it happened in: rebinding the pane to another slot must
  // neither show A's failure over B's thread nor forget it — it is shown again
  // when A is back on screen. Dismiss clears only the shown slot's entry.
  ...
  useEffect(() => {
    ...
    if (prev !== slotKey) {
      ...
      const incoming = readPaneDraft(slotKey)
      ...
      const parked = readPaneDraft(slotKey)
      if (parked.text) setInput(prev => mergeRecoveredDraft(prev, parked.text))
  ...
  /** Report an upload failure to the slot whose files failed. While the pane
   *  is mounted the per-slot banner map keeps it for that slot (shown when the
   *  slot is on screen, now or after the user returns). Once the pane is gone
   *  there is no banner to keep, so the failure goes into that slot's
   *  transcript as an error row — the same place a failed send reports — where
   *  the user finds it on return. Never dropped. */
  const reportUploadFailure = useCallback((message: string, forSlot: string) => {
    if (mountedRef.current) { setUploadError(message, forSlot); return }
  ...
  return (
    <SlotProvider slotId={slotKey}>
      ...
      >
        ...
        {/* The shared error surface. No agent hand-off (`askAgent` off): the
            composer next to it holds an unsaved draft, which the hand-off's
            navigation would destroy. */}
        <ErrorNotice
          message={uploadError}
          onDismiss={() => setUploadError('')}
          className="mx-4 mt-2"
          testId="pane-upload-error"
        />
