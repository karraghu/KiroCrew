...
- **Timeout**: 3 hours per subagent task (`agent.subagent_timeout_secs`), 20 minutes delivery (semaphore wait + injection), 15 minutes per injection attempt (`KIROCREW_INJECTION_TIMEOUT`, clamped to the delivery cap). A blocking `spawn_sub_agents` call is additionally bounded by `agent.chat_turn_timeout_secs` (2 hours by default), so raise that too when a batch must be awaited inside one turn
