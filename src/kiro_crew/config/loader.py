...
class KiroCrewConfig:
    ...
    def save(self) -> None:
        ...
        **The write happens under the sidecar advisory lock** — the same
        ...
        still publishes this object's in-memory snapshot, so callers that
        need read-modify-write semantics belong on
        :func:`update_config_locked` instead.
        ...
        **Async callers must offload.** A contended POSIX ``flock`` blocks
        ...
        reverted the first attempt at this lock (#4371). Every coroutine
        reaches ``save()`` through ``dashboard/chat_utils.run_config_write``
        (which also takes the loop-side asyncio config lock first, in the
        documented order) or ``asyncio.to_thread``; never inline.
        """
