...
async def _do_agents_sync(request: web.Request) -> web.Response:
    ...
    try:
        ...
        # A STARRED package crew is also kept: the star is the user saying
        # "this row matters to me", and pruning it would discard that
        # preference -- a reinstall re-adds the crew un-starred, since the add
        # path above builds a fresh record. Keeping the row costs one config
        # entry; the crew itself is still gone until reinstalled, and the star
        # is the user's to remove. (The re-add path is untouched: it only runs
        # for names NOT already in cfg.agents, so a kept row is not duplicated.)
        if discovered_names:
            for name, agent_cfg in list(cfg.agents.items()):
                if (
                    agent_cfg.source in ("package", "aim")
                    and agent_cfg.kiro_agent not in discovered_names
                    and not agent_cfg.starred
                ):
                    del cfg.agents[name]
                    pruned.append(name)
    except Exception:
