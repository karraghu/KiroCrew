...
Usage:
    python3 round_notes.py init  [--intent TEXT]
...
def safe_component(text: str) -> str:
    """A filesystem-safe name that stays DISTINCT for distinct inputs.

    Sanitising alone collapses ``feat/x`` and ``feat-x`` onto one file, so two
    loops would then share (and ``rm`` each other's) note. The short digest of
    the original text keeps them apart.
    """
    slug = re.sub(r"[^A-Za-z0-9._-]+", "-", text).strip("-") or "unnamed"
    return "{}-{}".format(slug, hashlib.sha1(text.encode("utf-8")).hexdigest()[:8])


def note_path(repo: str, branch: str) -> Path:
    return notes_root() / safe_component(repo) / (safe_component(branch) + ".md")
...
def resolve(args: argparse.Namespace) -> tuple[Path, str, str]:
    ...
    if branch in BASE_BRANCHES:
        sys.stderr.write(
            "round_notes: refusing to keep a note for the base branch {!r}.\n".format(branch)
        )
        sys.exit(2)
...
def cmd_init(args: argparse.Namespace) -> int:
    ...
    if not intent:
        sys.stderr.write("round_notes: no intent - pass --intent.\n")
        return 2
...
def cmd_add(args: argparse.Namespace) -> int:
    ...
    if not path.exists():
        sys.stderr.write(
            "round_notes: no note for this branch - run `round_notes.py init` first.\n"
        )
        return 2
...
def cmd_prune(args: argparse.Namespace) -> int:
    ...
    for path in root.rglob("*.md"):
        try:
            ...
            continue  # a sibling loop pruned it first - same outcome
        if stale:
